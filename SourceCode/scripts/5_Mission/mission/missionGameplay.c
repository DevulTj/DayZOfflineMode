...
class MissionGameplay extends MissionBase
{
	...
	void SpawnPlayer()
	{
		...
		vector player_pos = positions.Get(index);

		m_oPlayer = PlayerBase.Cast( GetGame().CreatePlayer( NULL, GetGame().CreateRandomPlayer(), player_pos, 0, "NONE") );
		
		GetGame().SelectPlayer( NULL, m_oPlayer );

		TStringArray tops = {"TacticalShirt_Grey","TacticalShirt_Black","TacticalShirt_Olive","TacticalShirt_Tan"};
		TStringArray pants = {"CargoPants_Green", "CargoPants_Black", "CargoPants_Blue", "CargoPants_Beige", "CargoPants_Grey"};
		TStringArray shoes = {"CombatBoots_Black","CombatBoots_Green","CombatBoots_Grey","CombatBoots_Beige","CombatBoots_Brown"};


		EntityAI item = m_oPlayer.GetInventory().CreateInInventory(tops.GetRandomElement());
		EntityAI item2 = m_oPlayer.GetInventory().CreateInInventory(pants.GetRandomElement());
		EntityAI item3 = m_oPlayer.GetInventory().CreateInInventory(shoes.GetRandomElement());
		
		item = m_oPlayer.GetInventory().CreateInInventory( "TacticalGloves_Green" );
		
		item = m_oPlayer.GetInventory().CreateInInventory( "CoyoteBag_Green" );
		item = m_oPlayer.GetInventory().CreateInInventory( "SmershVestBackpack" );
